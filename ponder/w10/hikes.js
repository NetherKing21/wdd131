...
searchBtn.addEventListener("click", e => {
  ...
  // Filter hike data based on user input
  const filtered = hikes.filter(hike => {
    return hike.name.toLowerCase().includes(userInput) || hike.description.toLowerCase().includes(userInput);
  });
  // Sort hikes by difficulty
  filtered.sort(sortHikesByDifficulty);
