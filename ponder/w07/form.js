...
form.addEventListener("submit", function (event) {
  ...
  // Let the user know to select at least one campus
    if (travelRange === "one" && selectedCampuses.length == 0) {
        output.textContent = "Select at least one campus";
        return;
    }

  // Let the user know if they choose many campuses but didn't put a note that they need to add a note
    if (travelRange === "many") {
       output.textContent = "Provide notes about the travel you're willing to do";
       return;
    }
  ...
  //Let the user know if they choose many campus but only had one campus selected that they need to choose at least two campuses
    if (travelRange === "many" && selectedCampuses.length < 2) {
        output.textContent = "Select at least two campuses";
        return;
    }
